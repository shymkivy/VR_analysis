...
from f_functions import load_XML, if_spheric_to_cart, if_cart_to_spheric, f_plot_volt #, f_pickle_load

#%%
dpath = path1 + '/UNITY/data_out2/'


fname_xml = 'Terrain3data_2025_4_22_10h_58m_0s.xml' #'Terrain3data_2025_3_17_11h_57m_58s.xml'

fname_loc = 'tracking_2025_3_13_11h_46m_0s.csv'# 'tracking_2025_3_13_11h_46m_0s.csv'
...
if has_daq:
    ...
    f_plot_volt(df2.data_in0, df2.Time); plt.title('data0')
    f_plot_volt(df2.data_in1, df2.Time); plt.title('data1')
    f_plot_volt(df2.data_in4, df2.Time); plt.title('data4')
...
ax1.plot([mouse_xyz[x_pt], mouse_xyz[x_pt] + mouse_dir_xyz[x_pt]*d], [mouse_xyz[z_pt], mouse_xyz[z_pt] + mouse_dir_xyz[z_pt]*d], 'o-', color='white')
ax1.plot([mouse_xyz[x_pt], mouse_xyz[x_pt] + mouse_dir_xyz_r[x_pt]*h_adj], [mouse_xyz[z_pt], mouse_xyz[z_pt] + mouse_dir_xyz_r[z_pt]*h_adj], 'o-', color='white')
ax1.plot([mouse_xyz[x_pt], mouse_xyz[x_pt] + mouse_dir_xyz_l[x_pt]*h_adj], [mouse_xyz[z_pt], mouse_xyz[z_pt] + mouse_dir_xyz_l[z_pt]*h_adj], 'o-', color='white')
...
# objects
ax1.plot(obj_x, obj_z, 'o', color='orange')
# path
ax1.plot(df.x_pos, df.z_pos, color='tab:blue')
...
for n_pt in range(200, 250):
    ...
    obj_dir2 = pd.DataFrame(obj_dir)
    obj_dir2.columns = ['x_pos', 'y_pos', 'z_pos']
    
    
    #x = np.exp(1j*phi)
    #phi2 = pd.Series(np.angle(np.exp(1j*phi)), name='phi')
    
    obj_dir_sph = if_cart_to_spheric(obj_dir2.x_pos, obj_dir2.y_pos, obj_dir2.z_pos)
