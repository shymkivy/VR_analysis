...
from f_functions import f_load_bh_data_all, f_get_session_data, f_plot_session2, f_proc_movement, f_proc_lick_rew, f_proc_lick_rew_df, f_comp_FOV_adj, f_add_phase, f_get_monitor_coords, f_angles_to_movie, f_angles_to_movie_v2 #, f_plot_session
...
if 0:
    pulse_diff_frames = np.array([bh['max_pulse_diff_frames'] for bh in bh_data])
    pulse_diff_vid    = np.array([bh['max_pulse_diff_vid']    for bh in bh_data])

    plt.figure()
    plt.plot(pulse_diff_frames - pulse_diff_vid, '.')
    plt.xlabel('dataset')
    plt.ylabel('frame - vid pulse diff (sec)')

    plt.figure()
    plt.plot(pulse_diff_vid, pulse_diff_frames - pulse_diff_vid, '.')
    plt.ylabel('frame - vid pulse diff (sec)')
...
if 0:
    ...
    ax2.plot(mov_data['time'][1:], mov_data['d_dist']+1)
    ax2.plot(mov_data['time'][1:], mov_data['d_phi']*2)
    ax2.set_xlabel('time (sec)')
    ax2.legend(['distance', 'rotation'])
    ax3.plot(mov_data['time'], np.sign(lr_data['lick_trace']), color=lick_col2)
    ax3.plot(mov_data['time'], np.sign(lr_data['rew_trace']), color=rew_col2)
    ...
    ax1.set_ylabel('CS sorted neurons')
    ax2.plot(mov_data['time'][1:], mov_data['d_dist']+1)
    ax2.plot(mov_data['time'][1:], mov_data['d_phi']*3)
    ax2.set_xlabel('time (sec)')
    ax2.plot(mov_data['time'], np.sign(lr_data['lick_trace'])*0.8-2, color=lick_col2)
    ax2.plot(mov_data['time'], np.sign(lr_data['rew_trace'])*0.8-3, color=rew_col2)
    ax2.set_xlabel('time (sec)')
...
if 0:
    ...
    ax1.set_title(ftag2)
    ax1.set_ylabel('CS sorted neurons')
    ax2.plot(mov_data['time'][1:], mov_data['d_dist'])
    ax2.plot(mov_data['time'][1:], mov_data['d_phi'])
    ax2.set_xlabel('time (sec)')
    ax2.legend(['distance', 'rotation'])
    ...
    ax1.set_title(ftag2)
    ax1.set_ylabel('CS sorted neurons')
    ax2.plot(mov_data['time'], np.sign(lr_data['lick_trace']))
    ax2.plot(mov_data['time'], np.sign(lr_data['rew_trace']))
    ax2.set_xlabel('time (sec)')
    ax2.legend(['lick', 'reward'])
    
    plt.figure()
    plt.plot(mov_data['time'], lr_data['lick_trace'])

#%%
S_smn_shuff = f_circshift_rates(S_smn, min_shift=0, rng=np.random.default_rng(42))
...
mouse_xyz = np.array((mov_data_eye['x_pos'], mov_data_eye['y_pos'], mov_data_eye['z_pos'])).T
...
if 1:
    ...
    f_plot_lateral_over_time(vec_data_r, mov_data['time'], axis=ax1)
    f_plot_vertical_over_time(vec_data_r, mov_data['time'], cam_params, axis=ax2)
    f_plot_dist_over_time(vec_data_r, mov_data['time'], axis=ax3)
    ax1.set_title('Right monitor')
    
    fig, (ax1, ax2, ax3) = plt.subplots(3,1, sharex=True)
    f_plot_lateral_over_time(vec_data_l, mov_data['time'], axis=ax1)
    f_plot_vertical_over_time(vec_data_l, mov_data['time'], cam_params, axis=ax2)
    f_plot_dist_over_time(vec_data_l, mov_data['time'], axis=ax3)
    ...
    if 1:
        ...
        left_mon_frames = f_angles_to_movie_v2(vec_data_l, mov_data['time'], cam_params, obj_size, lat_samp = num_samp, vert_samp = num_samp)
        right_mon_frames = f_angles_to_movie_v2(vec_data_r, mov_data['time'], cam_params, obj_size, lat_samp = num_samp, vert_samp = num_samp)
...
if 0:
    ...
    ax1.set_title(ftag2)
    ax1.set_ylabel('CS sorted neurons')
    f_plot_lateral_over_time(vec_data_r, mov_data['time'], axis=ax2)
    #f_plot_vertical_ov_time(mouse_xyz, mon_l_phi, theta, obj_locs, mov_data['time'], cam_params, axis=ax2)
    f_plot_dist_over_time(vec_data_r, mov_data['time'], axis=ax3)
    ax4.plot(mov_data['time'], np.sign(lr_data['lick_trace']), color=lick_col2)
    ax4.plot(mov_data['time'], np.sign(lr_data['rew_trace']), color=rew_col2)
    ...
    ax1.set_title(ftag2)
    ax1.set_ylabel('CS sorted neurons')
    f_plot_lateral_over_time(vec_data_l, mov_data['time'], axis=ax2)
    ax2.set_ylabel('Left mon')
    f_plot_lateral_over_time(vec_data_r, mov_data['time'], axis=ax3)
    ...
    f_plot_lateral_over_time(vec_data_l, mov_data['time'], axis=ax[1], xlabel=False)
    ax[1].set_ylabel('Left mon')
    f_plot_dist_over_time(vec_data_l, mov_data['time'], axis=ax[2], xlabel=False)
    f_plot_lateral_over_time(vec_data_r, mov_data['time'], axis=ax[3], xlabel=False)
    ax[3].set_ylabel('Right mon')
    f_plot_dist_over_time(vec_data_r, mov_data['time'], axis=ax[4], xlabel=False)
    ax[5].plot(mov_data['time'], np.sign(lr_data['lick_trace']), color=lick_col2)
    ax[5].plot(mov_data['time'], np.sign(lr_data['rew_trace']), color=rew_col2)
    ax[5].set_xlabel('time (sec)')
    ax[5].legend(['lick', 'reward'], loc='lower right')
    
    
    fig, ax = plt.subplots(7,1, sharex=True)
    f_plot_lateral_over_time(vec_data_l, mov_data['time'], axis=ax[0], xlabel=False)
    ax[0].set_title('Left monitor')
    f_plot_vertical_over_time(vec_data_l, mov_data['time'], cam_params, axis=ax[1], xlabel=False)
    f_plot_dist_over_time(vec_data_l, mov_data['time'], axis=ax[2], xlabel=False)
    f_plot_lateral_over_time(vec_data_r, mov_data['time'], axis=ax[3], xlabel=False)
    ax[3].set_title('Right monitor')
    f_plot_vertical_over_time(vec_data_r, mov_data['time'], cam_params, axis=ax[4], xlabel=False)
    f_plot_dist_over_time(vec_data_r, mov_data['time'], axis=ax[5], xlabel=False)
    ax[6].plot(mov_data['time'], np.sign(lr_data['lick_trace']), color=lick_col2)
    ax[6].plot(mov_data['time'], np.sign(lr_data['rew_trace']), color=rew_col2)
